...
class CrayPENvidia(CrayPECompiler):
    """Support for using the Cray Nvidia compiler wrappers."""
    PRGENV_MODULE_NAME_SUFFIX = 'nvidia'  # PrgEnv-nvidia
    COMPILER_FAMILY = TC_CONSTANT_NVHPC

    def __init__(self, *args, **kwargs):
        """CrayPENvidia constructor."""
        super(CrayPENvidia, self).__init__(*args, **kwargs)
        self.COMPILER_UNIQUE_OPTION_MAP['openmp'] = 'mp'
        for precflag in self.COMPILER_PREC_FLAGS:
            self.COMPILER_UNIQUE_OPTION_MAP[precflag] = NVHPC.COMPILER_UNIQUE_OPTION_MAP[precflag]
